...
const STATUS_COLS: { key: TaskStatus; label: string; color: string; bg: string; accent: string; canDropInto: boolean }[] = [
      ...
      <div className="hidden lg:block flex-1 overflow-x-auto p-6">
        {isError ? (
          ...
          <div className="flex gap-3 h-full" style={{ minWidth: '900px' }}>
            {grouped.map(col => (
              <div key={col.key}
                className={`flex-1 flex flex-col min-w-[175px] rounded-xl p-2 border border-gray-200 border-t-4 ${col.accent} transition-colors duration-150 ${
